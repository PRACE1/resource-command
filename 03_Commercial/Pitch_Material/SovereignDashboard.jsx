/**
 * SovereignDashboard.jsx — v2.1
 * Sovereign Mineral Audit Terminal
 *
 * Visual overhaul:
 *   1. Bloom post-processing on 3D lattice (graceful no-op if package absent)
 *      → npm install @react-three/postprocessing postprocessing
 *   2. Node size 0.065 → 0.14, point opacity maxed, scene fog for depth
 *   3. Background #0f172a → #090e1a (near-black), card top-accent stripes
 *   4. Telemetry hero values 21px → 30px
 *   5. Central QuorumRing visualization above proof stream
 *   6. Vignette opacity 0.5 → 0.28
 *
...
const C = {
  bg:          '#090e1a',   // near-black — cards now pop against this
  bgCard:      '#0d1b2a',
  bgBorder:    '#1e293b',
  emerald:     '#10b981',
  emeraldDim:  '#059669',
...
const AUDIT_PACK = [
  ...
  { label: 'ToB Engagement',          pct: 45,  done: false },
...
function SceneBloom() {
  ...
  useEffect(() => {
    import('@react-three/postprocessing')
      .then(m => setMods(m))
      .catch(() => {}) // silently disabled if package absent
  }, [])
  if (!Mods) return null
...
export function KineticLattice() {
  ...
  // Node size increased to 0.14 — previously 0.065 rendered sub-pixel at some viewports
  ...
  useEffect(() => {
    return () => {
      pointsGeo.dispose(); linesGeo.dispose()
...
// ─── UI: QUORUM RING ─────────────────────────────────────────────────────────
// Central SVG visualization — validator status ring + live consensus stats.
// Replaces blank center area above the proof stream.
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '22px' }}>
      ...
      <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ flexShrink: 0 }}>
        ...
        />
        {/* Pct */}
        <text x={cx} y={cy - 8} textAnchor="middle" fill={C.white} fontSize="22" fontWeight="700" fontFamily="JetBrains Mono, monospace" style={{ filter: `drop-shadow(0 0 5px ${ringColor})` }}>
          {pct}%
        </text>
        <text x={cx} y={cy + 8} textAnchor="middle" fill={C.slate} fontSize="7" fontFamily="Inter, sans-serif" letterSpacing="2.5">
          QUORUM
        </text>
        <text x={cx} y={cy + 20} textAnchor="middle" fill={ringColor} fontSize="6" fontFamily="Inter, sans-serif" fontWeight="700" letterSpacing="1.5">
          {hqr ? 'HQR SATISFIED' : 'HQR DEGRADED'}
        </text>
        {/* Validator dots around ring */}
        {vLabels.map((label, i) => {
          ...
          return (
            <g key={label}>
              <circle cx={nx} cy={ny} r="4" fill="none" stroke={col} strokeWidth="1.5"
                style={{ filter: ok ? `drop-shadow(0 0 4px ${ringColor})` : 'none' }} />
              <circle cx={nx} cy={ny} r="1.8" fill={col} />
              <text x={nx} y={ny + 12} textAnchor="middle" fill={col} fontSize="5.5"
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '13px', flex: 1 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '22px', fontWeight: 700, color: C.white, fontFamily: "'JetBrains Mono', monospace", textShadow: `0 0 12px ${C.emerald}90` }}>
            {round != null ? `#${String(round).padStart(5, '0')}` : '—'}
          </div>
        </div>
...
function VipReplyModal({ reply, onClose }) {
  ...
  async function handleApprove() {
    ...
    try {
      ...
      setErr(`Backend error ${e.message} — response not sent`)
      setSending(false)
    }
  }
      ...
      <div style={{ width: '560px', maxWidth: '92vw', background: 'linear-gradient(145deg, #07101f, #0d1b2a)', border: `1px solid ${C.red}55`, borderRadius: '12px', boxShadow: `0 0 70px ${C.red}18, 0 0 0 1px ${C.red}35`, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 18px', borderBottom: `1px solid ${C.bgBorder}` }}>
          ...
          <textarea value={edited} onChange={e => setEdited(e.target.value)} rows={4}
            style={{ width: '100%', background: `${C.emerald}06`, border: `1px solid ${C.emerald}28`, borderRadius: '6px', padding: '10px 13px', fontSize: '10.5px', color: C.slateLight, fontFamily: "'JetBrains Mono', monospace", lineHeight: '1.6', resize: 'vertical', outline: 'none', transition: 'border-color 0.2s' }}
...
// ─── UI: TELEMETRY WIDGET ────────────────────────────────────────────────────
// Hero values now 30px — readable from across a room
...
// ─── UI: CARD ────────────────────────────────────────────────────────────────
// accent prop controls the top gradient stripe — default emerald
...
function Card({ title, badge, badgeAccent = C.emerald, accent = C.emerald, children, style = {} }) {
  return (
    <div style={{ background: 'linear-gradient(135deg, rgba(9,14,26,0.94), rgba(13,27,42,0.97))', border: `1px solid ${C.bgBorder}`, borderRadius: '8px', backdropFilter: 'blur(14px)', overflow: 'hidden', ...style }}>
      {/* Coloured top stripe — creates depth against the #090e1a background */}
...
export default function SovereignDashboard() {
  ...
  const { lines, wsConnected }                          = useProofStream()
  ...
  const streamBadge  = wsConnected ? '● LIVE' : '◌ SIM'
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: C.bg, overflow: 'hidden', display: 'flex', flexDirection: 'column', fontFamily: 'Inter, sans-serif', position: 'relative' }}>
      ...
      <div ref={headerRef} style={{ padding: '11px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(7,10,22,0.96)', backdropFilter: 'blur(12px)', zIndex: 10, flexShrink: 0, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '11px' }}>
          ...
          <span style={{ fontSize: '8.5px', color: C.slate, letterSpacing: '0.8px' }}>
            Sovereign Mineral Audit Terminal · v2.1 · Region: SADC
          </span>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        {/* 3D Canvas — opaque, controls its own background color */}
        <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
          <Canvas
            ...
            frameloop="always"
          >
            {/* Scene background + atmospheric fog for depth */}
        ...
        {/* Vignette — reduced to 0.28 so lattice edges stay visible */}
        ...
        <div ref={leftRef} style={{ width: '238px', flexShrink: 0, padding: '14px 11px', zIndex: 3, display: 'flex', flexDirection: 'column', gap: '10px', overflowY: 'auto' }}>
          <Card title="Network Health" badge={backendOnline ? 'LIVE' : 'SIM'} badgeAccent={backendOnline ? C.emerald : C.amber} accent={C.emerald}>
            ...
            <TelemetryWidget label="Consensus Round"   value={status?.consensus_round ?? '—'} unit="blocks" status="ok" sub={`Last commit: ${v.last_commit_sec ?? '2.1'}s ago`} />
